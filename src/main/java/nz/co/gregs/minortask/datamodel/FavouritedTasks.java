 ...
 */
@DBRequiredTable
public class FavouritedTasks extends DBRow{
	...
	@DBColumn
	public final DBDate favouritedDate = new DBDate();

	public FavouritedTasks(Task task, User user) {
		taskID.setValue(task.taskID);
		userID.setValue(user.getUserID());
		favouritedDate.setValue(new Date());
	}

	public FavouritedTasks() {
	}
}
